...
namespace nvinfer1
{
    YoloLayerPlugin::YoloLayerPlugin(int classCount, int netWidth, int netHeight, int maxOut, const std::vector<YoloKernel>& vYoloKernel)
    {
        ...
        size_t AnchorLen = sizeof(float)* config.numAnchor * 2;
    ...
    YoloLayerPlugin::YoloLayerPlugin(const char* name, const void* data, size_t length)
    {
        using namespace Tn;
        ...
        d += kernelSize;
        CUDA_CHECK(cudaMallocHost(&mAnchor, mKernelCount * sizeof(void*)));
        size_t AnchorLen = sizeof(float)* ANCHOR_NUM * 2;
        for (int ii = 0; ii < mKernelCount; ii++)
        {
            CUDA_CHECK(cudaMalloc(&mAnchor[ii], AnchorLen));
            const auto& yolo = mYoloKernel[ii];
    ...
    int YoloLayerPlugin::getNbOutputs() const {
        return 1;
    }
    ...
    {
        assert((type == DataType::kHALF || type == DataType::kFLOAT) && format == PluginFormat::kNCHW);
    }
    ...
    __global__ void CalDetection(const float *input, float *output, int noElements,
		...
		const float anchors[ANCHOR_NUM * 2], int classes, int outputElem, float ignoreThresh)
    {
        ...
        const float* curInput = input + bnIdx * (info_len_i * total_grid * ANCHOR_NUM);

        for (int k = 0; k < ANCHOR_NUM; ++k) {
            float box_prob = Logist(curInput[idx + k * info_len_i * total_grid + 4 * total_grid]);
            if (box_prob < ignoreThresh) continue;
            int class_id = 0;
        ...
        for (unsigned int i = 0; i < mYoloKernel.size(); ++i) {
            ...
            CalDetection << < (numElem + mThreadCount - 1) / mThreadCount, mThreadCount, 0, stream >> >
                (inputs[i], output, numElem, mYoloV5NetWidth, mYoloV5NetHeight, 
				 mMaxOutObject, yolo.width, yolo.height, (float*)mAnchor[i], mClassCount, outputElem, config.ignoreThresh);
        }
    }
    ...
    IPluginExt* YoloPluginCreator::deserializePlugin(const char* name, const void* serialData, size_t serialLength)
    {
